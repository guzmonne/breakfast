...
var toggleActiveFilter = function(){
  var newClass, activeClass = Session.get('activeFilter');
  newClass = (activeClass === 'default') ? 'inverse' : 'default';
  Session.set('activeFilter', newClass);
...
Template.admin.helpers({
  ...
  activeFilter: function () { return Session.get('activeFilter'); },
});

Template.admin.onRendered(function () {
  this.$('.toggle-filter').slideToggle('slow');
});
...
Template.admin.events({
  ...
  'click #toggle_filter': function (e, template) {
    e.preventDefault();
    template.$('.toggle-filter').slideToggle('slow');
    toggleActiveFilter();
  },
