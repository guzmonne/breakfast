...
Clients.after.insert(function(userId, client){
  var body = [
	...
	'<h1 ',
		...
		'¡Inscripción Exitosa!',
	'</h1>',
        '<p style="font-family: arial, helvetica, sans-serif; line-height: 28px; font-size: 24px; display: block">Felicitaciones <b>' + client.name +'!</b> Ha realizado su registro con exito.</p>',
        '<p style="font-family: arial, helvetica, sans-serif; line-height: 28px; font-size: 24px; display: block;">Su número para el sorteo es el:</p>',
        '<h2 style="font-family: arial, helvetica, sans-serif; font-size: 45px; margin-top: 23px; margin-bottom: 11.5px; font-weight: 400; line-height: 1.1; color: #444444">' + client.count + '</h2>',
      '</div>',
    '</div>'
  ].join('');

  Email.send({
    from    : "no-reply@oktobertech.conatel.com.uy",
    to      : client.email,
    subject : "Oktobertech - Número de Sorteo",
    html    : body
  });
